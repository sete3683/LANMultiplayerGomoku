...
namespace Core
{
    ...
    public class SendBuffer : Buffer
    {
        ...
        public ArraySegment<byte> Open(int count)
        {
            ...
            return new ArraySegment<byte>(_buffer.Array, _buffer.Offset + _writePos, count);
        }

        public ArraySegment<byte> Close(int count)
        {
            ArraySegment<byte> segment = new ArraySegment<byte>(_buffer.Array, _buffer.Offset + _writePos, count);
            _writePos += count;
